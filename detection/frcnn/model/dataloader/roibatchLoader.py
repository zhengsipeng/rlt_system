...
class roibatchLoader(data.Dataset):
    def __init__(self, roidb, ratio_list, ratio_index, batch_size, num_classes, training=True, normalize=None):
        ...
        #self.hw_list_batch = torch.Tensor([self.data_size, 2]).zero_()
        num_batch = int(np.ceil(len(ratio_index) / batch_size))
        for i in range(num_batch):
            left_idx = i * batch_size
            right_idx = min((i + 1) * batch_size - 1, self.data_size - 1)
            #target_hw = self.hw_list[left_idx]
            ...
            #self.hw_list_batch[left_idx: (right_idx + 1)] = target_hw
    ...
    def __getitem__(self, index):
        ...
        if self.training:
            ...
            #print('data_size', data.size(1), data.size(2), ratio)
            if self._roidb[index_ratio]['need_crop']:
                if ratio < 1:
                    ...
                    # this means that data_width >> data_height, we need to crop the data_width
            ...
            #print(blobs['image'])
            #print('padding', padding_data.shape)
